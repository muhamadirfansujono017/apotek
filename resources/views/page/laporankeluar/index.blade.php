                ...
                <table class="min-w-full table-auto text-sm text-left">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-600">
                        @forelse ($barangKeluar as $i => $keluar)
                            @php 
                                // Menggunakan nama relasi detailPenjualan sesuai Model kamu
                                ...
                                @foreach ($rincian as $j => $item)
                                    <tr class="hover:bg-orange-50/30 dark:hover:bg-gray-700/50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-gray-700 dark:text-gray-300 border-l border-gray-50 dark:border-gray-700">
                                            {{-- Sesuaikan 'obat->nama_obat' dengan relasi di Model DetailPenjualan --}}
                                            {{ $item->obat->nama_obat ?? '-' }}
                                        </td>
                                        ...
                                        @if ($j === 0)
                                            <td class="px-6 py-4 text-right font-bold text-gray-900 dark:text-white" rowspan="{{ $rowspan }}">
                            ...
                            @else
                                <tr>
                                    <td class="px-6 py-4 text-center" colspan="3">{{ $barangKeluar->firstItem() + $i }}</td>
                                    <td colspan="3" class="px-6 py-4 text-center italic text-gray-400">Data rincian tidak ditemukan</td>
                                </tr>
                            @endif
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-gray-400 italic">Belum ada transaksi penjualan terekam.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
